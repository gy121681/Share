...
import com.shareshenghuo.app.shop.network.bean.GetShopPayInfoBean;
...
import com.shareshenghuo.app.shop.network.request.CardQuickPayRequset;
import com.shareshenghuo.app.shop.network.request.FenfuPayRequset;
import com.shareshenghuo.app.shop.network.request.MyBankCardRequest;
import com.shareshenghuo.app.shop.network.request.UpdateUserInfoRequest;
...
import com.shareshenghuo.app.shop.network.response.BankCardListResponse;
import com.shareshenghuo.app.shop.network.response.GetShopPayInfoResponse;
import com.shareshenghuo.app.shop.network.response.bankCardQuickPayResponse;
import com.shareshenghuo.app.shop.networkapi.Api;
import com.shareshenghuo.app.shop.util.ProgressDialogUtil;
import com.shareshenghuo.app.shop.util.T;
import com.shareshenghuo.app.shop.util.Util;
import com.shareshenghuo.app.shop.util.ViewUtil;
import com.shareshenghuo.app.shop.widget.dialog.BankListChoiceDialog;
import com.shareshenghuo.app.shop.widget.dialog.EidtDialog;
import com.shareshenghuo.app.shop.widget.dialog.OnEditDialogChlicListener;
...
public class PaymentnewActivity extends BaseTopActivity implements OnClickListener{
	...
	protected void onCreate(Bundle arg0) {
		...
		totamt = getIntent().getStringExtra("balance");
...
//		}
		...
		try {
			float bala1 = Float.parseFloat(balance.getText().toString());
			Log.e("", "bala1 = "+bala1);
//			double bala = Double.parseDouble(balance.getText().toString());
			if(bala1<=0){
				T.showShort(getApplicationContext(), "金额有误");
...
//		String url = Api.HOST+"jdPay/qrCodeUnifiedPayForShop?"+"shopId="+UserInfoManager.getUserInfo(this).shop_id+
//				"&ids="+ids+"&cardid="+banknum+"&truename="+user_names+"&cre_id="+
//				cre_ids+"&money="+String.valueOf((int)(Double.parseDouble(balance.getText().toString())*100));
		
//		String url = Api.HOST+"/oneCity/ytPay/qrCodeUnifiedPayForShop?"+"shopId="+UserInfoManager.getUserInfo(this).shop_id+
//				"&ids="+ids+"&cardid="+banknum+"&truename="+user_names+"&cre_id="+
//				cre_ids+"&money="+String.valueOf((int)(Double.parseDouble(balance.getText().toString())*100));
		String url = Api.HOST+"/oneCity/tongLianPay/shopPay?"+"shopId="+UserInfoManager.getUserInfo(this).shop_id+
				"&ids="+ids + "&money="+String.valueOf((int)(Double.parseDouble(balance.getText().toString())));
...
//		String url = Api.HOST+"/oneCity/ytPay/qrCodeUnifiedPayForShop?"+"shopId="+UserInfoManager.getUserInfo(this).shop_id+
//				"&ids="+ids+"&money="+String.valueOf((int)(Double.parseDouble(balance.getText().toString())*100));
		String url = Api.HOST+"/oneCity/tongLianPay/shopPay?"+"shopId="+UserInfoManager.getUserInfo(this).shop_id+
				"&ids="+ids+"&money="+String.valueOf((int)(Double.parseDouble(balance.getText().toString())*100));
		Intent about = new Intent(PaymentnewActivity.this, WebLoadActivity.class);
		about.putExtra("title", "充值");
...
//							startActivity(new Intent(PaymentnewActivity.this,Realname3Activity.class));
//							downloadDialog.dismiss();
//						default:
//							break;
//						}
