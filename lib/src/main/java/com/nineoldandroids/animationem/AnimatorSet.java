...
/**
 * This class plays a set of {@link Animatorem} objects in the specified order. Animations
 * can be set up to play together, in sequence, or after a specified delay.
 *
 * <p>There are two different approaches to adding animations to a <code>AnimatorSet</code>:
 * either the {@link AnimatorSet#playTogether(Animatorem[]) playTogether()} or
 * {@link AnimatorSet#playSequentially(Animatorem[]) playSequentially()} methods can be called to add
 * a set of animations all at once, or the {@link AnimatorSet#play(Animatorem)} can be
 ...
 */
public final class AnimatorSet extends Animatorem {
     ...
     */
    private ArrayList<Animatorem> mPlayingSet = new ArrayList<Animatorem>();
     ...
     */
    private HashMap<Animatorem, Node> mNodeMap = new HashMap<Animatorem, Node>();
     ...
     */
    public void playTogether(Animatorem... items) {
        if (items != null) {
            mNeedsSort = true;
     ...
     * @param items The animations that will be started simultaneously.
     */
    public void playTogether(Collection<Animatorem> items) {
        if (items != null && items.size() > 0) {
            mNeedsSort = true;
            Builder builder = null;
            for (Animatorem anim : items) {
                if (builder == null) {
                    builder = play(anim);
                } else {
                    builder.with(anim);
     ...
     */
    public void playSequentially(Animatorem... items) {
        if (items != null) {
            mNeedsSort = true;
            if (items.length == 1) {
                play(items[0]);
     ...
     * @param items The animations that will be started one after another.
     */
    public void playSequentially(List<Animatorem> items) {
        if (items != null && items.size() > 0) {
            mNeedsSort = true;
            if (items.size() == 1) {
                play(items.get(0));
     ...
     */
    public ArrayList<Animatorem> getChildAnimations() {
        ArrayList<Animatorem> childList = new ArrayList<Animatorem>();
    ...
    public void setTarget(Object target) {
        for (Node node : mNodes) {
            Animatorem animation = node.animation;
     ...
     */
    public Builder play(Animatorem anim) {
        if (anim != null) {
            mNeedsSort = true;
    ...
    public void start() {
        ...
        if (mStartDelay <= 0) {
            ...
            mDelayAnim.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationCancel(Animatorem anim) {
                    canceled = true;
                }
                public void onAnimationEnd(Animatorem anim) {
                    if (!canceled) {
    ...
    public AnimatorSet clone() {
        ...
        anim.mStarted = false;
        anim.mPlayingSet = new ArrayList<Animatorem>();
        anim.mNodeMap = new HashMap<Animatorem, Node>();
    ...
    private static class DependencyListener implements AnimatorListener {
         ...
         */
        public void onAnimationCancel(Animatorem animation) {
        }

        /**
         * An end event is received - see if this is an event we are listening for
         */
        public void onAnimationEnd(Animatorem animation) {
         ...
         */
        public void onAnimationRepeat(Animatorem animation) {
        }

        /**
         * A start event is received - see if this is an event we are listening for
         */
        public void onAnimationStart(Animatorem animation) {
         ...
         */
        private void startIfReady(Animatorem dependencyAnimation) {
    ...
    private static class Node implements Cloneable {
         ...
         */
        public Node(Animatorem animation) {
        ...
        public Node clone() {
            try {
                ...
                node.animation = (Animatorem) animation.clone();
                return node;
     ...
     * AnimatorSet#play(Animatorem) play()} method of <code>AnimatorSet</code> is to make it possible
     * to express the dependency relationships of animations in a natural way. Developers can also
     * use the {@link AnimatorSet#playTogether(Animatorem[]) playTogether()} and {@link
     * AnimatorSet#playSequentially(Animatorem[]) playSequentially()} methods if these suit the need,
     * but it might be easier in some situations to express the AnimatorSet of animations in pairs.
     * <p/>
     * <p>The <code>Builder</code> object cannot be constructed directly, but is rather constructed
     * internally via a call to {@link AnimatorSet#play(Animatorem)}.</p>
     * <p/>
     ...
     * </pre>
     * <p/>
     * <p>Note in the example that both {@link Builder#before(Animatorem)} and {@link
     * Builder#after(Animatorem)} are used. These are just different ways of expressing the same
     * relationship and are provided to make it easier to say things in a way that is more natural,
     * depending on the situation.</p>
     * <p/>
     * <p>It is possible to make several calls into the same <code>Builder</code> object to express
     * multiple relationships. However, note that it is only the animation passed into the initial
     * {@link AnimatorSet#play(Animatorem)} method that is the dependency in any of the successive
    ...
    public class Builder {
         ...
         * {@link AnimatorSet#play(Animatorem)} call that created this <code>Builder</code> object.
         *
         * @param anim The animation that will play when the animation supplied to the
         * {@link AnimatorSet#play(Animatorem)} method starts.
         */
        public Builder with(Animatorem anim) {
         ...
         *
         * @param anim The animation that will play when the animation supplied to the
         * {@link AnimatorSet#play(Animatorem)} method ends.
         */
        public Builder before(Animatorem anim) {
            Node node = mNodeMap.get(anim);
            if (node == null) {
                node = new Node(anim);
                mNodeMap.put(anim, node);
         ...
         * {@link AnimatorSet#play(Animatorem)} method to play.
         */
        public Builder after(Animatorem anim) {
            Node node = mNodeMap.get(anim);
            if (node == null) {
                node = new Node(anim);
                mNodeMap.put(anim, node);
