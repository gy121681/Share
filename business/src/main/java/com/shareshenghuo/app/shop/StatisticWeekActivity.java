 ...
 *	周数据明细
 */
public class StatisticWeekActivity extends BaseTopActivity {
	...
	public void loadData() {
		...
		req.type = from+"";
		req.query_type = QUERY_TYPE.WEEK;
		...
		new HttpUtils().send(HttpMethod.POST, api, params, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> resp) {
				...
				if(from == FROM.COUPON_USE) {
					...
					if(Api.SUCCEED == bean.result_code) {
						...
						lvData.setAdapter(new CouponWeekStatisticAdapter(StatisticWeekActivity.this, bean.data.result_list));
					} else {
						T.showShort(StatisticWeekActivity.this, bean.result_desc);
					}
				} else {
					StatisticMonthResponse bean = new Gson().fromJson(resp.result, StatisticMonthResponse.class);
					if(Api.SUCCEED == bean.result_code) {
						setText(R.id.tvDataTotal, bean.data.all_fee+"");
						lvData.setAdapter(new StatisticMonthAdapter(StatisticWeekActivity.this, bean.data.result_list));
					} else {
						T.showShort(StatisticWeekActivity.this, bean.result_desc);
					}
				}
			...
			public void onFailure(HttpException arg0, String arg1) {
				ProgressDialogUtil.dismissProgressDlg();
				T.showNetworkError(StatisticWeekActivity.this);
			}
		});
	}
}
