...
public abstract class CommonDialog implements OnDismissListener{
	...
	public void showAtBottom1() {
		setWindowAnimation(R.style.anim_slide_bottom);
		show1(Gravity.BOTTOM,null);
	}
	...
	public void show(int gravity, View parent) {
		if(dlg != null) {
			...
			dlg.showAtLocation(parent, 0, 0, gravity);
//			dlg.showAtLocation(parent, gravity, 0, gravity);
			setBackgroundAlpha(context, 0.7f);
		}
		Utility.hideSoftInput((Activity) context);
	}
	
	public void show1(int gravity, View parent) {
		if(dlg != null) {
			if(!isInitComplete) {
				initDlgView();
				isInitComplete = true;
	...
	public void setImageByURL(int viewId, final String url) {
		ImageView iv = getView(viewId);
		ImageLoadManager.getInstance(context).displayImage(url, iv);
	}
