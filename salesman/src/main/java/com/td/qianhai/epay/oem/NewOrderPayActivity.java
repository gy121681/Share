...
public class NewOrderPayActivity extends BaseActivity implements OnClickListener {
	/** 金额、付款卡号、付款卡号开户姓名 */
	...
	private  String banknum,c_num,bakname,cardtag,mobile; 
	
	private void initView() {
		Intent it = getIntent();
		banknum = it.getStringExtra("FRPID");
		c_num = it.getStringExtra("CARDCODE");
		bakname = it.getStringExtra("ISSUER");
		cardtag = it.getStringExtra("CRDFLG");
...
//		custId = ((AppContext) getApplication()).getCustId();
		...
		InputMethodUtils.show(NewOrderPayActivity.this, etBalance);
...
//						String b = nstr[1].substring(0,2);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		share = MyCacheUtil.getshared(NewOrderPayActivity.this);
...
//				ToastCustom.showMessage(this, "充值金额不能小于1");
	...
	private void receive() {
		...
		if (balance != null && !balance.equals("")) {
			float intBalance = Float.parseFloat(balance);
			if (intBalance < 3) {
				Toast.makeText(getApplicationContext(),"充值金额不能小于3元",
...
//				ToastCustom.showMessage(this, "充值金额不能小于2.00元");
				...
				etBalance.setText("");
				Toast.makeText(getApplicationContext(),"充值金额不能大于10000元,请重新输入",
						Toast.LENGTH_SHORT).show();
...
//			Intent it = new Intent(NewOrderPayActivity.this,PayMainActivity.class);
//			
...
//		});
	...
	class OrderTask extends AsyncTask<String, Integer, HashMap<String, Object>> {
		...
		protected void onPreExecute() {
			AlertDialog.Builder builder = new Builder(NewOrderPayActivity.this);
		...
		protected void onPostExecute(HashMap<String, Object> result) {
			...
			if (result != null) {
				if (Entity.STATE_OK.equals(result.get(Entity.RSPCOD))) {
					Intent intent = new Intent(NewOrderPayActivity.this,
							OnlineWeb.class);
					...
					ToastCustom.showMessage(NewOrderPayActivity.this,
	...
	public void onClick(View v) {
		if (DateUtil.isFastDoubleClick()) {
			...
			case R.id.bt_title_right:
				...
				Intent it = new Intent(NewOrderPayActivity.this,
	...
	Runnable run = new Runnable() {
		...
		public void run() {
			...
			try {
				...
				String[] values = {banknum,c_num,bakname,balances,mobile,cardtag,""};
				result = NetCommunicate.executeHttpPostnewpay(HttpUrls.NEWPAY,
						HttpKeys.NEWPAY_BACK,HttpKeys.NEWPAY_ASK,values);
...
//			
	...
	public void handleMessage(Message msg) {
		switch (msg.what) {
		case 1:
			if(result.get("returnUrl")!=null){
				String url = result.get("returnUrl").toString();
				Intent it = new Intent(NewOrderPayActivity.this,NewWebViewActivity.class);
				it.putExtra("URLs", url);
				startActivity(it);
				finish();
			}
		...
		case 2:
			warnDialog = new OneButtonDialogWarn(NewOrderPayActivity.this,
...
Runnable run1 = new Runnable() {
	...
	public void run() {
		...
		try {
			...
			String[] values = {result.get("requestType").toString(),
								result.get("requestId").toString(),
								result.get("totalBizType").toString(),
								result.get("merchantCode").toString(),
								result.get("totalPrice").toString(),
								result.get("passThrough").toString(),
								result.get("goodsDesc").toString(),
								result.get("rePayTimeOut").toString(),
								result.get("noticeUrl").toString(),
								result.get("encode").toString(),
								result.get("successReturnUrl").toString(),
								result.get("failReturnUrl").toString(),
								result.get("backUrl").toString(),
								result.get("bankAccount").toString(),
								result.get("cardNumberLock").toString(),
								result.get("productId").toString(),
								result.get("productName").toString(),
								result.get("fund").toString(),
								result.get("merAcct").toString(),
								result.get("bizType").toString(),
								result.get("productNumber").toString(),};
			result = NetCommunicate.executeHttpPostnewpay(HttpUrls.NEWPAY,
					HttpKeys.NEWPAY_BACK,HttpKeys.NEWPAY_ASK,values);
		} catch (HttpHostConnectException e) {
			// TODO Auto-generated catch block
