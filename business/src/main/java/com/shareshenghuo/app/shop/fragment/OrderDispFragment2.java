...
public class OrderDispFragment2 extends BaseFragment implements OnClickListener,OnRefreshListener2<ScrollView>{
...
//		initTopBar("订单详情");
		...
		adapter = new OrderdetailsListAdapter(activity, data,1);
...
//						String url = Api.wxHOST+"oneCity/ytPay/qrCodeUnifiedPayForShop?"+"shopId="+UserInfoManager.getUserInfo(activity).shop_id+
//						"&ids="+buff.toString()+"&money="+String.valueOf((int)(Double.parseDouble(balance)*100));
						String url = Api.wxHOST+"oneCity/tongLianPay/shopPay?"+"shopId="+UserInfoManager.getUserInfo(activity).shop_id+
						"&ids="+buff.toString()+"&money="+String.valueOf((int)(Double.parseDouble(balance)*100));
