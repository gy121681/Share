...
public class MyEditDialog extends Dialog implements OnClickListener {
	...
	/** 标题、内容 */
	private TextView tv_title, tv_money, tv_yuan, tv_tran;
	/** 确认、取消 */
	...
	public static String teshu = "";// 自定义,teshu的话.说明是wyp创建的
	...
	public MyEditDialog(Context context, int theme, String title,
			...
			String ratemoney, OnMyDialogClickListener onMyDialogClickListener,
			onMyaddTextListener onMyListener) {
		super(context, theme);
		this.context = context;
		this.title = title;
	...
	private void initview() {
		...
		if (isTrans) {
			...
			tv_tran.setVisibility(View.GONE);
			tv_yuan.setVisibility(View.VISIBLE);
			if (teshu.equals("my_buy")) {
				tv_yuan.setText("投资:");
			}
		}
	...
	public boolean dispatchKeyEvent(KeyEvent event) {
		if (event.getKeyCode() == KeyEvent.KEYCODE_DEL
			...
			if (paypwd.isFocused()) {
				...
				if (paypwd5.getText().toString() != null
						&& !paypwd5.getText().toString().equals("")) {
					paypwd5.setText("");
				} else {
					// paypwd4.setEnabled(true);
					paypwd5.clearFocus();
