 ...
 *月数据明细
 */
public class StatisticMonthActivity extends BaseTopActivity {
	...
	public void loadData() {
		...
		req.type = from+"";
		req.query_type = QUERY_TYPE.MONTH;
		...
		new HttpUtils().send(HttpMethod.POST, api, params, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> resp) {
				...
				if(from == FROM.COUPON_USE) {
					...
					if(Api.SUCCEED == bean.result_code) {
						...
						lvData.setAdapter(new CouponWeekStatisticAdapter(StatisticMonthActivity.this, bean.data.result_list));
					} else {
						T.showShort(StatisticMonthActivity.this, bean.result_desc);
					}
				} else {
					StatisticMonthResponse bean = new Gson().fromJson(resp.result, StatisticMonthResponse.class);
					if(Api.SUCCEED == bean.result_code) {
						setText(R.id.tvDataTotal, bean.data.all_fee+"");
						lvData.setAdapter(new StatisticMonthAdapter(StatisticMonthActivity.this, bean.data.result_list));
					} else {
						T.showShort(StatisticMonthActivity.this, bean.result_desc);
					}
				}
			...
			public void onFailure(HttpException arg0, String arg1) {
				ProgressDialogUtil.dismissProgressDlg();
				T.showNetworkError(StatisticMonthActivity.this);
			}
		});
	}
}
